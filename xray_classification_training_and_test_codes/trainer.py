...
class Trainer():
	def __init__(self, args, my_loader, my_model, my_optimizer, my_checkpoint):
		...
		self.savedir = os.path.join('../MIT_Save_FinalCVPR/', self.args.exp_name)
	...
	def train_Classifier(self):
		...
		for i_batch, (xrayimages, xraylabels, xraylabels_uncer) in enumerate(self.loader_train):

			xrayimages, xraylabels, xraylabels_uncer = xrayimages.cuda(), xraylabels.cuda(), xraylabels_uncer.cuda()
			self.optimizer.zero_grad()
			out = self.model(xrayimages/255.0, 'classifier')
	...
	def test_Classifier(self):
		...
		val_loss = 0
		GT, PRED, UN = [], [], []
		for i_batch, (xrayimages, IMlabel, IMlabel_uncer) in enumerate(self.loader_valid):

			sys.stdout.write("Calculation in Progress: %d/ %d  \r" % (i_batch+1, len(self.loader_valid)))
			sys.stdout.flush()

			xrayimages, IMlabel, IMlabel_uncer = xrayimages.cuda(), IMlabel.cuda(), IMlabel_uncer.cuda()
